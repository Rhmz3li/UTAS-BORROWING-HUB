...
const ResourceDetail = () => {
  ...
  const isDeptMismatch = () => {
    if (!resource?.department || resource.department.trim() === '') return false;
    if (!user || ['Admin', 'Assistant'].includes(user.role)) return false;
    const userDept = (user.department || '').trim().toLowerCase();
    const resDept = resource.department.trim().toLowerCase();
    return !userDept || userDept !== resDept;
  };
              ...
              {((resource.status === 'Available' || resource.status === 'Reserved') && resource.available_quantity > 0) && (
                <div className="d-flex gap-3" style={{ marginTop: '1.5rem' }}>
                  <Button
                    ...
                    title={isDeptMismatch() ? `Only available for ${resource.department} department` : 'Borrow this resource now'}
                    style={{
                      flex: 1,
                      fontWeight: '600',
                      background: isDeptMismatch() ? '#c8c8c8' : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
                      border: 'none',
                      borderRadius: '15px',
                      padding: '1rem',
                      fontSize: '1.1rem',
                      boxShadow: isDeptMismatch() ? 'none' : '0 6px 20px rgba(102, 126, 234, 0.4)',
                      transition: 'all 0.3s ease',
                      cursor: isDeptMismatch() ? 'not-allowed' : 'pointer',
                      opacity: isDeptMismatch() ? 0.6 : 1
                    }}
                    onMouseEnter={(e) => {
                      if (!isDeptMismatch()) {
                  ...
                  <Button
                    ...
                    title={isDeptMismatch() ? `Only available for ${resource.department} department` : '⏳ Reserve - Requires Admin Approval'}
                    style={{
                      flex: 1,
                      fontWeight: '600',
                      background: isDeptMismatch() ? '#c8c8c8' : 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)',
                      border: 'none',
                      borderRadius: '15px',
                      padding: '1rem',
                      fontSize: '1.1rem',
                      boxShadow: isDeptMismatch() ? 'none' : '0 6px 20px rgba(79, 172, 254, 0.4)',
                      transition: 'all 0.3s ease',
                      cursor: isDeptMismatch() ? 'not-allowed' : 'pointer',
                      opacity: isDeptMismatch() ? 0.6 : 1
                    }}
